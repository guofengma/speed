...
function getData(){
	$.getJSON(ctx+'/order/qcOrder_listDo', {page: 1,qcOrderNum:qcOrderNum,qcUserId:qcUserId}, function(res){ //从第6页开始请求。返回的json格式可以任意定义
		laypage.render({
			elem: 'page', //容器。值支持id名、原生dom对象，jquery对象。【如该容器为】：&lt;div id="page1">&lt;/div>
	        count: res.count, //通过后台拿到的总页数
	        jump: function(e){ //触发分页后的回调
	        	$.getJSON(ctx+'/order/qcOrder_listDo', {page: e.curr,qcOrderNum:qcOrderNum,qcUserId:qcUserId}, function(res){
	                //渲染
	                ...
	                for(var o in res.result){  
	                	i = i+1;  	                	
	                	if($("#qcId").val()==res.result[o].qcUserId){
	                		content = content + 
			                "<tr class='text-c'>"
			                /*+"<td>"+"<input type='checkbok' name='ids'/ value='isNull(res.result[o].id)'>"+"</td>"*/
			                +"<td>"+"<input type='checkbox' name='ids' checked='checked' value="+isNull(res.result[o].id)+" class='ids'/>"+i+"</td>" 
	                		...
	                		content = content + 
			                "<tr class='text-c'>"			             
			                +"<td>"+"<input type='checkbox' name='ids' value="+isNull(res.result[o].id)+" class='ids'/>"+i+"</td>" 
			                +"<td>"+isNull(res.result[o].qcOrderNum)+"</td>"
			                +"<td>"+isNull(res.result[o].taskGoal)+"</td>"
			                +"<td>"+isNull(res.result[o].clientName)+"</td>"
			                +"<td>"+isNull(res.result[o].factoryName)+"</td>"
...
function select(){
	//多选框取选中值
     ...
     console.log("ids:"+chestr+""+"idNos:"+noKed)     
	$.ajax({
		data:{'ids':chestr,'idNos':noKed,'id':$("#qcId").val(),'username':$("#qcName").val()},
		url: '${ctx}/qcUser_pipeiDo',
		dataType: 'json',
		success: function(data){
			if (data.status == 0){
				...
				layer.msg('匹配失败!',{icon:1,time:1000});
			}
		},
		error:function(data) {
