...
function getData(){
	$.getJSON(ctx+'/base/numUnit_listDo', {page: 1}, function(res){ //从第6页开始请求。返回的json格式可以任意定义
		laypage.render({
			elem: 'page', //容器。值支持id名、原生dom对象，jquery对象。【如该容器为】：&lt;div id="page1">&lt;/div>
	        count: res.count, //通过后台拿到的总页数
	        jump: function(e){ //触发分页后的回调
	            $.getJSON(ctx+'/base/numUnit_listDo', {page: e.curr}, function(res){
	                //渲染
	                ...
	                +"<td>"+isNull(res.result[o].unitName)+"</td>"
...
function add(){
	var url = ctx+"/base/numUnit_add";
	layer_show('添加单位',url,700,300);
}

function edit(id){
	var url = ctx+"/base/numUnit_edit?id="+id;
	layer_show('编辑单位',url,1100,500);
}
