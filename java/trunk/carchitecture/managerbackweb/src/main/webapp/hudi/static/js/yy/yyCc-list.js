...
function getData(){
	$.getJSON(ctx+'/yy/yyCc_listDo', {page: 1}, function(res){ //从第6页开始请求。返回的json格式可以任意定义
		laypage.render({
			elem: 'page', //容器。值支持id名、原生dom对象，jquery对象。【如该容器为】：&lt;div id="page1">&lt;/div>
	        count: res.count, //通过后台拿到的总页数
	        jump: function(e){ //触发分页后的回调
	            $.getJSON(ctx+'/yy/yyCc_listDo', {page: e.curr}, function(res){
	                //渲染
...
function add(){
	var url = ctx+"/yy/yyCc_add";
	layer_show('添加尺寸项目',url,700,300);
}

function edit(id){
	var url = ctx+"/yy/yyCc_edit?id="+id;
	layer_show('编辑尺寸项目',url,1100,500);
}
