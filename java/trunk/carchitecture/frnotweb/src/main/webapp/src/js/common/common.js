function util(){
    ...
    this.set_footer_tabbar_color = function(num,link){
        ...
        /*$('.footer_tabbar_box').find('.footer_tabbar_item').eq(num).addClass('active'+num+'').siblings().removeClass('active');*/
        if(link !== null){
     ...
     * @param {string} link 默认 null  导航对应名称
     */
    this.set_footer_tabbar_icon = function(num,iconOld,iconNew){
        inum = num || 0,iconOld = iconOld || null,iconNew = iconNew || null;
        console.log($('.footer_tabbar_box').find('.footer_tabbar_item').eq(num).children().eq(0).attr("class"));
        $('.footer_tabbar_box').find('.footer_tabbar_item').eq(num).children().eq(0).removeClass(iconOld);
        $('.footer_tabbar_box').find('.footer_tabbar_item').eq(num).children().eq(0).addClass(iconNew);
    }
    ...
    this.set_footer_tabbar_link = function(link){
        switch (link) {
            case 'home':
                ...
                break;
            case 'product':
                window.location.href = './../../html/product/product.html';
                break;
            case 'material':
            ...
            case 'cart':
                ...
                break;
            default:
                window.location.href = './../../html/my/my.html';
                break;
        }
    }
