class util{  
     ...
     */
    alert(text,title=null,callbackOk){
        if(title !== null){
    ...
    set_nav(){
        ...
        i = 0,
        nav = ''
        for(let val of nav_json){
            if(i >= 1)  active = ''
            i++
            nav += `<li id="${val.id}" class="footer_tabbar_item ${active}"><div class="footer_tabbar_item_img ${val.icon} icon"></div><div class="footer_tabbar_item_name">${val.name}</div></li>`
        }
     ...
     */
    set_footer_tabbar_color(num=0,link=null){
     ...
     */
    set_banner(element,data,isVertical=false,isLooping=true,isAutoplay=true){
        return new iSlider({
